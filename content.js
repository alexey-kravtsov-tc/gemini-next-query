...
// Session storage for this chat
const chatSession = {
    history: [], // Array of query arrays
    currentIndex: -1
};
...
function updatePaginationUI() {
    ...
    document.getElementById('prev-btn').onclick = () => {
        if (chatSession.currentIndex > 0) {
            chatSession.currentIndex--;
            renderButtons(chatSession.history[chatSession.currentIndex]);
            updatePaginationUI();
        }
    };
    document.getElementById('next-btn').onclick = () => {
        if (chatSession.currentIndex < chatSession.history.length - 1) {
            chatSession.currentIndex++;
            renderButtons(chatSession.history[chatSession.currentIndex]);
            updatePaginationUI();
        }
